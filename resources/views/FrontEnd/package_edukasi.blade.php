...
@section('content')
    ...
    <div id="container_Button" style="padding-top: 80px">
        ...
        <form action="{{route('package_education.main')}}">
            <button id="category_Button_Edukasi"class="active">Paket Edukasi</button>
        </form>
    </div>
    ...
    <div class="item_semua">
        ...
        @foreach ($educations as $education)
            <div class="container" id="container_Image">
                <img src="{{$education->image ? asset('img/' . $education->image) : 'https://placehold.co/400/orange/white?text=Food'}}" class="img-fluid">
            </div>

            <div class="container" id="container_Info">
                <h4>{{$education->name}}</h4>
                <p id="harga">Harga(per orang): {{'Rp. '. number_format($education->price, 0, ',', '.')}}<p>
                <p id="orang">Minimal orang: {{$education->people . ' orang'}}<p>
                <div class="fasilitas">
                    <p>
