...
@section('content')
    <div class="container-fluid px-4">
        <h1 class="mt-4 mb-4">Tambah Package</h1>
        <div class="card mb-4">
            <form action="{{ route('adventure.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        <h4 class="form-h4">Jumlah Orang</h4>
                        <input type="text" class="form-control {{ $errors->has('people') ? ' border-danger' : '' }}" name="people" value="{{ old('people') }}">
                        @error('people')
                            <span class="text-danger"> {{$message}} </span>
                        @enderror
                    </div>
