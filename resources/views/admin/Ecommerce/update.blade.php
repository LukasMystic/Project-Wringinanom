...
@section('content')
    <div class="container-fluid px-4">
        <h1 class="mt-4 mb-4">Edit Ecommerce</h1>
        <div class="card mb-4">
            <form action="{{ route('product.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        <img id="previewImage" src="{{ $product->image ? asset('img/' . $product->image) : 'https://placehold.co/300/orange/white?text=Food' }}" style="width: 30%; height: 30%;">
                    </div>
                    ...
                    <div class="mb-2">
                        <h4>Deskripsi</h4>
                        <textarea class="mySummernote" id="description" name="description"> {{$product->description}}</textarea>
                        @error('description')
                            <span class="text-danger"> {{$message}} </span>
                        @enderror
                    </div>
